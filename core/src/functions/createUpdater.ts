...
export const setter = <RootState extends RootStateType>(initialRootState: RootState) =>
    ...
    (acc, slice) => {
      acc[slice] = (payload, forceRenderer) => {
        ...
        getObjectKeys(payload).forEach(async (key) => {
          try {
            const cb: PayloadValue<RootState, typeof slice, keyof typeof payload> =
              payload[key]!;
            // if (isCallable(cb)) {
            const setter: (
              rootState: RootState
            ) => Promise<
              PayloadReturnValue<RootState, typeof slice, keyof typeof payload>
            > = async (rootState) =>
              isCallable(cb) ? await cb(rootState[slice][key], rootState) : cb;
